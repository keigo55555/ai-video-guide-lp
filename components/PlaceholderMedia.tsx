...
export default function PlaceholderMedia({ kind, label, source, poster, priority = false }: PlaceholderMediaProps) {
  ...
  if ((kind === "image" && sourceExists) || (!sourceExists && posterExists && poster)) {
    ...
    return (
      <div className="relative aspect-[16/9] overflow-hidden rounded-[2rem] border border-zinc-200 bg-zinc-100">
        <Image src={imageSource!} alt={label} fill className="object-cover" sizes="(max-width: 768px) 100vw, 1120px" priority={priority} />
      </div>
    );
  }
